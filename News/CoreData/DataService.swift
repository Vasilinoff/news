...
class DataService {
    ...
    func saveNewsListItem(_ newsListItem: NewsListItem) {
        if let context = coreDataStack.saveContext {
            context.perform { [weak self] in
                ...
                self?.performSave(context: context, completionHandler: { _,_ in })
            }
        }
    }
    ...
    func saveNewsItem(_ newsItem: NewsItem) {
        if let context = coreDataStack.saveContext {
            context.perform { [weak self] in
                ...
                self?.performSave(context: context, completionHandler: { _, error in
                })
            }
        }
    }
    ...
    func deleteAllNews() {
        let listRequest = NSFetchRequest<NSFetchRequestResult>(entityName: "NewsListItemSave")
        let deleteListRequest = NSBatchDeleteRequest(fetchRequest: listRequest)
        if let saveContext = coreDataStack.saveContext {
            do {
                try saveContext.persistentStoreCoordinator?.execute(deleteListRequest, with: saveContext)
            } catch {
                print("failed to delete NewsListItemSave")
            }
        }
        let itemRequest = NSFetchRequest<NSFetchRequestResult>(entityName: "NewsItemSave")
        let deleteItemRequest = NSBatchDeleteRequest(fetchRequest: itemRequest)
        if let saveContext = coreDataStack.saveContext {
            do {
                try saveContext.persistentStoreCoordinator?.execute(deleteItemRequest, with: saveContext)
            } catch {
                print("failed to delete NewsItemSave")
            }
        }
